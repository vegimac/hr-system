...
**Lohnlauf nur HR (Walter 17.09.2026, Filial-Schalter `lohnlauf_nur_hr`):** Default aus = GF + HR wie oben. Bei ein: GF-Buttons unsichtbar; HR bestätigt `BERECHNET` → `HR_BESTAETIGT` in einem Schritt; «An HR senden» entfällt. Erste HR-Bestätigung setzt Definitiv auf `provisorisch_abgeschlossen` (Edit-Sperre) bzw. Akonto auf `BEI_HR`. In der MA-Liste ein grünes ✓ «bestätigt» (kein Doppelhaken). Status-Map bleibt; nur die GF-Stufe wird übersprungen. Gilt Akonto und Definitiv.
