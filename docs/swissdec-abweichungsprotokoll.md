...
| **Formel (Anhang 1)** | **IST** = steuerbar (CH-Tage). **Satz-Lohn** = (Σ periodisch ÷ QST-Tage × 360 + Σ aperiodisch) ÷ 12. **Je Tarifcode ein Topf**; Steuer je Topf = Satz(Code, Satz-Lohn) × Topf **rappengenau**. Gerundet wird **nur der Monatsabzug** auf 5 Rp.; der Rest (kumulierte Steuer per Vormonat exakt − tatsächlich abgezogen) wird aus den Vormonats-Töpfen neu gerechnet (`JahressteuerExakt`), nicht gespeichert. |
...
| **Stand** | 1–2 Satz-Töpfe 20.09. morgens. 3 Töpfe/Code + QST-Tage 20.09. abends. QST Round05 (Topf ja, Monatsabzug nein; Monatsmodell + Pauschale + K1) 20.09. Umbuchung rückwirkender Versionen in die Töpfe + K1 als bezahlt 20.09. (Claude). Januar-Belege (10 TF) gegen Anhang 1 grün; Soll-Tabelle `SWISSCEC/Abgleich/qst_jahresmodell_soll.md`. Februar: 8/12 grün; Fixes 20.09. (Claude): Start/QST-Tage aus Vertragsdeckung (TF41), keine Kurzmonat-Hochrechnung im Jahresmodell (TF25/26), Sonderzahlungen mit Σ CH-Tage ausscheiden (TF29). |
| **5 Rp. (26.09.2026)** | Früher wurde **jeder Topf** auf 5 Rp. gerundet → bis zu 5 Rp. Abweichung im Monat. Über alle 122 Monate der Soll-Tabelle betraf es vier: TF22 Bucher **08** (309.65 statt 309.60) und **09** (388.40 statt 388.45), TF23 Koller **12** (1'014.35 statt 1'014.40), TF34 Rinaldi **09** (99.05 statt 99.10, Jahr 2'739.50 statt 2'739.55). Jetzt: Topf rappengenau, nur der Monatsabzug auf 5 Rp., Rest aus dem Vormonats-Stand gerechnet (`JahressteuerExakt` mit den HEUTE gültigen Codes — eine rückwirkende Umbuchung zählt nicht doppelt, weil der K1-Posten in «bezahlt» steckt). Bewusst **nicht** im Slip gespeichert: sonst wirkte die Regel erst, wenn das ganze Jahr neu gerechnet ist. So genügt der betroffene Monat. Tests in `Tests/QstJahresmodellTests.cs`. |
