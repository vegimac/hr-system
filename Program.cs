...
builder.Services.AddScoped<DarlehenVertragPdfService>();   // K3 MA-Darlehen (Walter 29.08.2026)
...
using (var scope = app.Services.CreateScope())
{
    ...
    // gleichzeitig offene Fenster. KEIN JSON-Default im SQL (geschweifte
    // Klammern = String.Format-Falle) — das EF-Model setzt «leer» selbst.
    // Platzierung wie beim Nummernkreis: VOR SchemaCheckService.Pruefe, sonst
    // meldet die Pruefung die Tabelle als fehlend und deploy.sh bricht ab.
    db.Database.ExecuteSqlRaw(@"
    ...
    ");
    // Funktion pro Vertragsabschnitt geprüft (Walter 22.09.2026): steuert die
    // Kontrollliste «Funktionen prüfen» — geprüfte Abschnitte erscheinen dort nicht mehr.
    db.Database.ExecuteSqlRaw(@"
    ...
    // Direkt verknüpfte Dokumente (Walter 23.09.2026): AHV-Karte, Geburtsurkunde
    // (MA + Familienmitglied), Zivilstandsdokument, Foto (Ausschnitt), Bankbeleg
    // pro Konto. Platzierung: VOR SchemaCheckService.Pruefe (Stolperfalle 9).
    db.Database.ExecuteSqlRaw(@"
        ALTER TABLE employee ADD COLUMN IF NOT EXISTS ahv_karte_dokument_id INTEGER;
        ALTER TABLE employee ADD COLUMN IF NOT EXISTS geburtsurkunde_dokument_id INTEGER;
        ALTER TABLE employee ADD COLUMN IF NOT EXISTS zivilstand_dokument_id INTEGER;
    ...
    db.Database.ExecuteSqlRaw(@"
        ...
        -- Dokument 1:1 pro Absenz, z.B. Arztzeugnis (Walter 23.09.2026)
    ...
    // Texte mit Namen und Personalnummern zusammen. Kategorien ohne Eintrag
    // fallen still auf ihr label aus dashboard_warning_config zurück — eine
    // neue Warnung kann also nie etwas kaputt machen.
    // Texte sind ein ENTWURF aus der Fachlogik heraus; Korrekturen laufen per
    // UPDATE auf der Tabelle, ohne Deploy.
    db.Database.ExecuteSqlRaw(@"
        CREATE TABLE IF NOT EXISTS todo_anleitung (
            id         SERIAL PRIMARY KEY,
    ...
    db.Database.ExecuteSqlRaw(@"
        ...
        ON CONFLICT (category) DO NOTHING;
        -- Walter 05.09.2026: Probezeit-Entscheid «Kündigung» gefällt, aber die
        -- Kündigung ist noch nicht erfasst → kritisch bis sie draussen ist.
        INSERT INTO dashboard_warning_config
            (category, label, enabled, warn_days, escalate_days, severity_base, severity_escalated, is_date_based, sort_order, todo_priority, warn_color)
        VALUES
            ('probezeit_kuendigung_offen', 'Kündigung in Probezeit erfassen', TRUE, NULL, NULL, 'critical', NULL, FALSE, 28, 21, 'red')
        ON CONFLICT (category) DO NOTHING;
        INSERT INTO todo_anleitung (category, titel, anleitung, sort_order) VALUES
        ('probezeit_kuendigung_offen',
         'Kündigung in der Probezeit erfassen',
         'Der Entscheid nach dem Probezeitgespräch lautet «Vertrag beenden». Die Kündigung muss dem Mitarbeiter VOR dem letzten Zustelltag zugehen (Probezeit-Ende minus Kündigungsfrist in der Probezeit) — sonst gilt danach die lange Frist. Mitarbeiter öffnen, Probezeit-Pille → «Kündigung während Probezeit», Kündigung erfassen und aushändigen.',
         155)
        ON CONFLICT (category) DO NOTHING;
    ...
    db.Database.ExecuteSqlRaw(@"
        ...
        FROM (VALUES
            ...
            ('FLEX',  '195.2', 'automatisch', 30),
            ('FLEX',  '195.1', 'saldo',       40),
            ('FLEX',  '40.1',  'ereignis',    50),
            ('FLEX',  '180.1', 'automatisch', 60),
            ('FLEX',  '70.1',  'ereignis',    70),
            ('FLEX',  '70.2',  'ereignis',    80),
            ('FLEX',  '60.2',  'ereignis',    90),
            ('FLEX',  '60.3',  'ereignis',   100),
            ('FLEX',  '110.1', 'ereignis',   110),
            ('FLEX',  '55.10', 'austritt',   120),
            ('FLEX',  '40.1',  'austritt',   130),
            ('FLEX',  '180.1', 'austritt',   140),
            -- MTP (garantierte Stunden)
            ('MTP',   '10.1',  'automatisch', 10),
            ('MTP',   '55.3',  'automatisch', 20),
            ('MTP',   '195.5', 'saldo',       30),
            ('MTP',   '10.2',  'ereignis',    40),
            ('MTP',   '195.4', 'automatisch', 50),
            ('MTP',   '195.4', 'ereignis',    60),
            ('MTP',   '180.1', 'saldo',       70),
            ('MTP',   '70.1',  'ereignis',    80),
            ('MTP',   '70.2',  'ereignis',    90),
            ('MTP',   '60.2',  'ereignis',   100),
            ('MTP',   '60.3',  'ereignis',   110),
            ('MTP',   '110.1', 'ereignis',   120),
            ('MTP',   '55.10', 'austritt',   130),
            ('MTP',   '40.1',  'austritt',   140),
            ('MTP',   '180.1', 'austritt',   150),
            -- FIX (Monatslohn)
            ('FIX',   '10.1',  'automatisch', 10),
            ('FIX',   '10.2',  'ereignis',    20),
            ('FIX',   '10.3',  'ereignis',    30),
            ('FIX',   '180.1', 'saldo',       40),
            ('FIX',   '75.1',  'ereignis',    50),
            ('FIX',   '70.1',  'ereignis',    60),
            ('FIX',   '70.2',  'ereignis',    70),
            ('FIX',   '65.1',  'ereignis',    80),
            ('FIX',   '60.2',  'ereignis',    90),
            ('FIX',   '60.3',  'ereignis',   100),
            ('FIX',   '110.1', 'ereignis',   110),
            ('FIX',   '40.1',  'austritt',   120),
            ('FIX',   '50.1',  'austritt',   130),
            ('FIX',   '55.10', 'austritt',   140),
            -- FIX-M (Management-Festlohn) — wie FIX; 13. ML gemaess Monatsraster
            ('FIX-M', '10.1',  'automatisch', 10),
            ('FIX-M', '10.2',  'ereignis',    20),
            ('FIX-M', '10.3',  'ereignis',    30),
            ('FIX-M', '180.1', 'saldo',       40),
            ('FIX-M', '75.1',  'ereignis',    50),
            ('FIX-M', '70.1',  'ereignis',    60),
            ('FIX-M', '70.2',  'ereignis',    70),
            ('FIX-M', '65.1',  'ereignis',    80),
            ('FIX-M', '60.2',  'ereignis',    90),
            ('FIX-M', '60.3',  'ereignis',   100),
            ('FIX-M', '110.1', 'ereignis',   110),
            ('FIX-M', '40.1',  'austritt',   120),
            ('FIX-M', '50.1',  'austritt',   130),
    ...
    // (Nettolohn-Verbindlichkeit) / Gegen 1140 (Vorschüsse und Darlehen an
    // Personal, KMU-Kontenrahmen — im Kontoplan-UI anpassbar). Idempotent.
    db.Database.ExecuteSqlRaw(@"
