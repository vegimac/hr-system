...
- Rollen: `admin` (alle Filialen), `superuser` (HR-Verantwortliche), `user` (normaler Benutzer/GF mit Filial-Zugang via `user_branch_access`), `employee` (MA-Postfach-Login — nur eigenes Postfach/Lohnzettel).
...
- **Saldo-Listen zum Definitiv-Abschluss (Walter-Vorgabe 21.05.2026):** `GET /api/payroll/saldo-liste-buchhaltung?companyProfileId&year&month` und `GET /api/payroll/saldo-liste-gf?...` — zwei A4-Quer-PDFs pro Filiale+Periode, on-demand aus den persistierten `PayrollSaldo`-Zeilen (Service `LohnSaldoListePdfService`, QuestPDF, analog `AkontoListePdfService`). Buchhaltung = alle Saldi (Std/Nacht/Ferien-Tage/Ferien-Geld/Feiertag-Tage/13.ML) + Brutto/Netto + IBAN + Summenzeile; GF = kompakte Übersicht, bei **UTP** ist die 13.-ML-Spalte „—" (13. wird monatlich ausbezahlt). Frontend: `payroll.js → lohnSaldoListe('buchhaltung'|'gf')`, Buttons „📊 Buchhaltung" / „📋 GF-Übersicht" in der Definitiv-Aktionszeile (`_lohnWfRenderStatusBar`, ab Status `provisorisch_abgeschlossen` + `abgeschlossen`). Download via `saveBlobAsk`.
