...
async function wcInit() {
    ...
    try {
        ...
        if (!r.ok) {
            cont.innerHTML = `<div style="padding:24px;color:#dc2626;font-size:13px">Konnte Warnungs-Konfiguration nicht laden (HTTP ${r.status}). Wurde die Migration <code>add_dashboard_warning_config.sql</code> ausgeführt?</div>`;
            return;
        }
...
function wcRender() {
    ...
    const rowsHtml = _wcRows.map((c, i) => {
        ...
        const escCell = dateBased
            ? `<input type="number" min="0" value="${c.escalateDays == null ? '' : c.escalateDays}"
                   ...
                   style="width:70px;padding:6px 8px;border:1px solid #d9d3ca;border-radius:8px;background:#faf8f5;font-size:13px;color:#3f3f3f">`
            : '<span style="color:#b8b0a4;font-size:12px">—</span>';
        // Eskalierter Schweregrad — Auswahl inkl. Leer-Option (keine Eskalation).
        const escSevSel = `
    ...
    cont.innerHTML = `
    <div style="background:rgba(255,255,255,0.5);border:1px solid rgba(255,255,255,0.62);border-radius:14px;box-shadow:0 6px 20px rgba(60,55,48,0.14);overflow:hidden">
    ...
    <p style="margin-top:12px;font-size:12px;color:#8b8b8b">
        Nur datums-basierte Warnungen (z.B. Bewilligung läuft ab) haben einen Vorlauf und eine Eskalations-Schwelle.
        Zustands-Warnungen (z.B. QST-Pflicht offen) sind immer aktiv-sofort mit ihrem Basis-Schweregrad.
    </p>`;
}

// Lokale Änderung im Cache (kein sofortiges Speichern).
...
async function wcSave() {
    ...
    try {
        ...
        // Neu laden, damit der Server-Stand wieder gilt.
        await wcInit();
    } catch (e) {
