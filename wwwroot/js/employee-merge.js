// ════════════════════════════════════════════════════════════════════════
// Duplikate bereinigen — mehrfache Employee-Einträge zur selben easy@work-ID
// zusammenführen (Walter-Vorgabe 21.06.2026). Einmalige Bereinigung.
// ════════════════════════════════════════════════════════════════════════
    ...
    box.innerHTML = _empMergeGroups.map((g, gi) => {
        ...
        <div style="border:1px solid #e2e8f0;border-radius:10px;padding:14px;margin-bottom:14px">
            <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin-bottom:8px">
                ...
                <span style="color:#94a3b8;font-size:12px">easy@work-ID ${g.easyAtWorkId} · ${g.employees.length} Einträge</span>
            </div>
            <table style="width:100%;border-collapse:collapse;font-size:13px">
                <thead><tr style="color:#64748b;text-align:left;border-bottom:1px solid #e2e8f0">
                    ...
                    <th style="padding:4px 8px">Personalnr.</th><th style="padding:4px 8px">Filialen</th>
                    <th style="padding:4px 8px">Eintritt</th><th style="padding:4px 8px">Austritt</th><th style="padding:4px 8px">Status</th>
                </tr></thead>
                <tbody>${rows}</tbody>
            </table>
...
async function empMergePreview(gi) {
    ...
    try {
        ...
        out.innerHTML = `<div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:8px;font-size:12px;color:#475569">
            ...
            <div>Als Alias gesichert: <strong>${(b.aliasNumbers||[]).map(escapeHtml).join(', ') || '–'}</strong></div>
