...
async function sollPdf() {
    const cp = (typeof fixedCompanyProfileId !== 'undefined') ? fixedCompanyProfileId : null;
    if (!cp) { alert('Bitte oben im Sidebar zuerst eine Filiale wählen.'); return; }
    const month = document.getElementById('sollMonth')?.value;
    const year  = document.getElementById('sollYear')?.value;
    const stich = document.getElementById('sollStichtag')?.value || '';
    if (!month || !year) { alert('Monat und Jahr wählen.'); return; }
    let url = `/api/payroll/sollstunden-report/pdf?companyProfileId=${cp}&year=${year}&month=${month}`;
    if (stich) url += `&stichtag=${encodeURIComponent(stich)}`;
    const fname = `Sollstunden_${year}-${String(month).padStart(2, '0')}.pdf`;
    try {
